...
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'my_super_secret_for_jwt_in_lab_4_default')
...
def create_json_response(data, status_code=200):
    ...
    return Response(json_data, mimetype='application/json; charset=utf-8', status=status_code)
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not token:
            return create_json_response({'message': 'Токен аутентификации отсутствует!'}, 401)
        
        try:
            data = jwt.decode(token, app.config['SECRET_KEY'], algorithms=["HS256"])
            current_user = api_users.get(data['user'])
            if not current_user:
                 return create_json_response({'message': 'Пользователь из токена не найден!'}, 401)
...
def login():
    ...
    if not user_data or not check_password_hash(user_data['password_hash'], auth.password):
        return create_json_response({'message': 'Неверное имя пользователя или пароль'}, 401)
...
def get_available_scooters(current_user):
    ...
    if current_user['role'] != 'client':
        return create_json_response({'message': 'Доступ запрещен: функция только для клиентов'}, 403)
    ...
    try:
        ...
        return create_json_response({'message': 'Внутренняя ошибка сервера при выполнении запроса'}, 500)
    finally:
        if session:
            session.close()
...
def get_maintenance_log(current_user):
    ...
    if current_user['role'] != 'technician':
        return create_json_response({'message': 'Доступ запрещен: функция только для техников'}, 403)
