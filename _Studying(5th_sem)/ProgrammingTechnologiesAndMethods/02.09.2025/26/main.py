...
def find_best_pair(lines):
    ...
    Args:
        lines (list[str]): Список строк из входного файла.

    Returns:
        tuple[int, int]: Кортеж с номером ряда и номером второго места в паре.

    Raises:
        ValueError: Если данные в файле некорректны.
        RuntimeError: Если подходящая пара не найдена (не должно происходить по ТЗ).
    """
    if not lines:
    ...
    # Этот блок является защитой на случай, если входные данные нарушат это условие.
    if best_r == 0:
...
def main():
    ...
    Обрабатывает аргументы, читает файл и выводит результат.
    """
    if len(sys.argv) != 2:
