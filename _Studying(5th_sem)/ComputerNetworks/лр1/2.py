...
def encode_4b5b(binary_msg):
    return ''.join(table_4b5b[binary_msg[i:i+4]] for i in range(0, len(binary_msg), 4))
...
def plot_signal(time, signal, title, bits=None):
    ...
    if bits is not None:
        for i, b in enumerate(bits):
            ...
            plt.text((i + 0.5) * T_ns, 1.2, b, ha='center', va='bottom', fontsize=7, color='blue')
    plt.show()
...
f_sr = (f_v + f_n) / 2
S = f_v - f_n
print(f"Манчестер (4B/5B): f_в={f_v/1e6:.1f} МГц, f_н={f_n/1e6:.1f} МГц, f_ср={f_sr/1e6:.1f} МГц, S={S/1e6:.1f} МГц")
print("-" * 30)
...
for bit in encoded_message:
    if bit == '0':
        ...
        last *= -1
        ami.append(last)
t_ami = np.linspace(0, len(encoded_message) * T_bit_prime, len(encoded_message) + 1) * 1e9
...
f_sr = (f_v + f_n) / 2
S = f_v - f_n
print(f"AMI (4B/5B, N={N_4b5b}): f_в={f_v/1e6:.1f} МГц, f_н={f_n/1e6:.1f} МГц, f_ср={f_sr/1e6:.1f} МГц, S={S/1e6:.1f} МГц")
print("-" * 30)