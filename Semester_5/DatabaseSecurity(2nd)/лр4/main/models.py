...
class Tariff(Base):
    ...
    startprice = Column(
        "StartPrice", Numeric(5, 2), nullable=False, server_default="0.00"
    )
...
class User(Base):
    ...
    registrationdate = Column(
        "RegistrationDate", DateTime, nullable=False, server_default=func.now()
    )
...
class Maintenance(Base):
    ...
    scooterid = Column(
        "ScooterID", Integer, ForeignKey("scooter.ScooterID"), nullable=False
    )
...
class Trip(Base):
    ...
    tariffid = Column(
        "TariffID", Integer, ForeignKey("tariff.TariffID"), nullable=False
    )
...
class Payment(Base):
    ...
    paymentdate = Column(
        "PaymentDate", DateTime, nullable=False, server_default=func.now()
    )
    paymentstatus = Column(
        "PaymentStatus", String(20), nullable=False, server_default="completed"
    )
...
def get_session(role: str, password: str):
    ...
    try:
        ...
        print(
            f"ОШИБКА: Не удалось подключиться к базе данных для пользователя '{role}': {e}"
        )
        return None
