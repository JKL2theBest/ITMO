...
plot_signal(
    t_m, manchester, "Манчестер (скремблированный код)", bits=first4_bytes_scrambled
)

f_v_man, f_n_man = C, C / 2
print(
    f"Манчестер (скрембл.): f_в={f_v_man/1e6:.1f} МГц, f_н={f_n_man/1e6:.1f} МГц, S={(f_v_man - f_n_man)/1e6:.1f} МГц"
)
print("-" * 40)
...
for bit in first4_bytes_scrambled:
    if bit == "0":
        ...
        ami.append(last)
t_ami = (
    np.linspace(0, len(first4_bytes_scrambled) * T_bit, len(first4_bytes_scrambled) + 1)
    * 1e9
)
plot_signal(
    t_ami, [ami[0]] + ami, "AMI (скремблированный код)", bits=first4_bytes_scrambled
)

# Ищем самую длинную серию нулей во ВСЕЙ последовательности
N_scrambled = max(
    len(list(g)) for k, g in itertools.groupby(scrambled_message) if k == "0"
)
...
print(
    f"AMI (скрембл., N={N_scrambled}): f_в={f_v_ami/1e6:.1f} МГц, f_н={f_n_ami/1e6:.1f} МГц, S={S_ami/1e6:.1f} МГц"
)
print("-" * 40)
