...
def generate_random_domain():
    domain = "".join(
        random.choices(string.ascii_lowercase + string.digits, k=random.randint(3, 63))
    )
    if random.choice([True, False]):
        domain += "." + "".join(
            random.choices(string.ascii_lowercase, k=random.randint(2, 6))
        )
    domain += "." + "".join(
        random.choices(string.ascii_lowercase, k=random.randint(2, 6))
    )
...
def main():
    ...
    parser.add_argument(
        "-n", "--num", type=int, default=None, help="количество строк для записи в файл"
    )
    parser.add_argument(
        "filename",
        nargs="?",
    ...
    try:
        ...
        data_offsets = [
            sum(len(s) for s in data_bytes[:i]) for i in range(len(data_bytes))
        ]
        index_area = struct.pack("H" * len(data_offsets), *data_offsets)
        offset_field = struct.pack("I", len(data_area))
        random_data_area = bytes(
            random.getrandbits(8) for _ in range(random.randint(0, 1000))
        )
