...
static void bfq_add_request(struct request *rq)
{
    ...
    bfq_log_bfqq(bfqd, bfqq, "Adding request %p, is_sync: %d, timestamp: %llu", rq, rq_is_sync(rq), now_ns);
...
static struct request *__bfq_dispatch_request(struct blk_mq_hw_ctx *hctx)
{
	...
	if (!list_empty(&bfqd->dispatch)) {
        ...
        bfq_log(bfqd, "Dispatching request %p, queue: %p, flags: %u",
                rq, bfqq, rq->rq_flags);
...
static void bfq_finish_request(struct request *rq)
{
    ...
    bfq_log(bfqd, "Finishing request %p, flags: %u", rq, rq->rq_flags);
