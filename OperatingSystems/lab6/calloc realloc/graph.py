...
def plot_comparison(malloc_data, other_data, title, ylabel):
    ...
    # Malloc
    plt.plot(
        malloc_data["Size(Bytes)"],
        malloc_data["TotalTime(us)"],
        marker="o",
        linestyle="-",
        label="Malloc Total Time",
    )
...
def plot_comparison_with_details(malloc_data, other_data, title, ylabel):
    ...
    for func, color in zip(["calloc", "realloc"], ["green", "orange"]):
        ...
        plt.plot(
            ...
            marker="^",
            linestyle="--",
            label=f"{func.capitalize()} Alloc Time",
            color=color,
        )
        ...
        plt.plot(
            ...
            marker="v",
            linestyle=":",
            label=f"{func.capitalize()} Free Time",
            color=color,
        )
...
plot_comparison_with_details(
    malloc_data, other_data, "Сравнение Malloc, Calloc и Realloc Times", "Время (мкс)"
)

plot_comparison(
    malloc_data,
    other_data,
    "Сравнение of Malloc, Calloc, and Realloc Total Time",
    "Время (мкс)",
)
